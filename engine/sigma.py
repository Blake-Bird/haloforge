...
def rho0_from_cosmology(cosmology: dict) -> float:
    ...
    _require_positive(cosmology["Omega_m"], "Omega_m")
...
def build_power_interpolator(
    ...
    k_arr, p_arr = _validate_power_grid(k, P, minimum_samples=2)
    logk = np.log(k_arr)
    logp = np.log(p_arr)

    def interpolator(k_value: float | np.ndarray) -> np.ndarray:
        kv = np.asarray(k_value, dtype=float)
        _require_positive(kv, "Interpolation wavenumber")
...
def mass_from_R(R: np.ndarray | float, rho0: float) -> np.ndarray:
    ...
    _require_positive(r, "Radius")
    _require_positive(rho0, "Mean density")
    return 4.0 * np.pi * float(rho0) * r**3 / 3.0


def R_from_mass(M: np.ndarray | float, rho0: float) -> np.ndarray:
    m = np.asarray(M, dtype=float)
    _require_positive(m, "Mass")
    _require_positive(rho0, "Mean density")
    return (3.0 * m / (4.0 * np.pi * float(rho0))) ** (1.0 / 3.0)


def _require_positive(values, name: str) -> None:
    values = np.asarray(values, dtype=float)
    if np.any(~np.isfinite(values)) or np.any(values <= 0):
        raise ValueError(f"{name} must be finite and strictly positive")


def _validate_power_grid(
    k: np.ndarray, P: np.ndarray, *, minimum_samples: int = 8
    ...
    if k_arr.size < minimum_samples:
        raise ValueError(f"At least {minimum_samples} k samples are required")
    if (
...
def _sharp_k_variance(R: np.ndarray, k: np.ndarray, P: np.ndarray) -> np.ndarray:
    """Integrate a log-linear P(k) exactly to min(1/R, k_max).

    On each interval P(k) is a power law, so k³P(k) has an analytic
    antiderivative in ln(k). Including the partial terminal interval avoids
    grid-dependent steps from applying Simpson's rule to a Heaviside window.
    No power is extrapolated beyond the sampled interval.
    """
    logk = np.log(k)
    base = k**3 * P / (2.0 * np.pi**2)
    width = np.diff(logk)
    slope = np.diff(np.log(base)) / width
    areas = base[:-1] * width * exprel(slope * width)
    cumulative = np.concatenate(([0.0], np.cumsum(areas)))
    cutoff = np.clip(-np.log(R), logk[0], logk[-1])
    index = np.clip(np.searchsorted(logk, cutoff, side="right") - 1, 0, k.size - 2)
    partial_width = cutoff - logk[index]
    return cumulative[index] + base[index] * partial_width * exprel(
        slope[index] * partial_width
    )


def integration_method(window_type: str) -> str:
    return (
        "piecewise power-law integral to sharp-k cutoff"
...
def sigma_squared(
    ...
    """Compute finite-range sigma² with a window-appropriate quadrature.
    ...
    _require_positive(R, "Radius")
    if window_type == "Sharp-k":
        value = float(_sharp_k_variance(np.asarray(R), k_arr, p_arr))
        if not np.isfinite(value):
            raise FloatingPointError("Sharp-k variance is non-finite")
        return value
    logk = np.log(k_arr)
    ...
    if value < 0:
        raise FloatingPointError("sigma integration returned a negative variance")
    return float(value)
...
def sigma_grid(
    ...
    if window_type == "Sharp-k":
        sigma2 = _sharp_k_variance(R, k_arr, p_arr)
    else:
    ...
    return {
        ...
        "rho0": rho0,
        "integration_method": integration_method(window_type),
    }
...
def dlog_sigma_dlog_M(M_values: np.ndarray, sigma_values: np.ndarray) -> np.ndarray:
    ...
    if M.ndim != 1 or M.size < 2 or sigma.shape != M.shape:
        raise ValueError(
            "Derivative requires at least two matching mass and sigma samples"
        )
    _require_positive(M, "Mass")
    ...
    return np.gradient(np.log(sigma), np.log(M), edge_order=edge_order)
...
def compute_sigma_result(power_result: dict, params: dict) -> dict:
    ...
    focus_index = redshift_index(redshifts, params.get("single_z", 0.0))
    ...
    return {
        ...
        "integration_method": grid["integration_method"],
